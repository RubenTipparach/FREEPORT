...
| `driving-hud.html` | the driver's HUD (speed, fuel as range, wallet, next pump, frame counter, a compass strip to the first marker) over a moving road, and the MAP: a plan of the roads, towns and pumps where a click sets numbered route markers and the legs are summed against the tank; A* between markers is the feature after it | https://claude.ai/artifact/2i1EWdyyK5pkHbxB1KH16P |
