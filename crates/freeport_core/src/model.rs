...
use crate::dc::DcMesh;
use crate::field::{hash3, Block, CONCRETE, GLASS, LAMP, LIT, PLATE, STREET};
...
/// How far a street's paving stands over the ground it is laid on,
...
/// audit's own number); five centimetres clears that by twenty five times
/// and is under any step, so the paving needs no collider of its own and
/// a walker never sinks into a kerb.
const LIFT: f64 = 0.05;
...
/// A piece of street: one quad of paving, laid `LIFT` over the levelled
/// ground and colliding with nothing, because it stands under any step.
pub fn street(w: f64, d: f64) -> Model {
    let mut m = Model::new();
    let (hw, hd) = (w * 0.5, d * 0.5);
    let z = DVec3::Z * LIFT;
    m.quad(
        DVec3::new(-hw, -hd, 0.0) + z,
        DVec3::new(hw, -hd, 0.0) + z,
        DVec3::new(hw, hd, 0.0) + z,
        DVec3::new(-hw, hd, 0.0) + z,
        STREET,
